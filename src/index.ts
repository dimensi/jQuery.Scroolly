...
scroolly._isObject = (val) => typeof val === 'object';

scroolly._isArray = (val) => val instanceof Array;

scroolly._isNumber = (val) => val instanceof Number || typeof val === 'number';

scroolly._isString = (val) => val instanceof String || typeof val === 'string';

scroolly._default = (obj, key, defaultValue) => {
 ...
 */
scroolly.parseCoords = (boundry) => {
  const strings = boundry.split(/\s*=\s*/);
  const coordRel = strings[0] || 'doc-top';
  const parsedCoordRel = scroolly.parseCoord(coordRel);
  const coordVP = strings[1] || parsedCoordRel.anchor;
  const parsedCoordVP = scroolly.parseCoord(coordVP);
 ...
 * @return {object} - parsed boundry
 */
scroolly.parseCoord = (coord) => {
  const reAnchor = /((vp|doc|el|con)-)?(top|center|bottom)?/i;
  const reOffsetStr = '(\\+|-)?\\s*(\\d+)(\\%|vp|doc|el|con)?';
  let reOffset = new RegExp(reOffsetStr, 'gi');
  const mA = coord.match(reAnchor);
  const mO = coord.match(reOffset);

  if (!mA && !mO) {
    return false;
  }

  const subject = mA[1] ? mA[2] : 'vp';
  const anchor = mA[3] || 'top';
  const offsets = [];
 ...
 * @return {object} - parsed boundry
 */
scroolly.calculateCoord = (coord, $element, $container) => {
  ...
  if ('vp' === coord.subject) {
    ...
    const $subject = 'con' === coord.subject ? $container : $element;
    const subjectHeight = $subject.outerHeight();
    const subjectTop = $subject.offset().top;
    const subjectBottom = subjectTop + subjectHeight;
    const subjectCenter = subjectTop + Math.floor(subjectHeight / 2);
 ...
 */
scroolly.cmpCoords = (coords, $element, $container) =>
 ...
 * @return {boolean}
 */
scroolly.isRuleInActiveWidthRange = (rule) => {
  const fromX = scroolly._default(rule, 'minWidth', 0);
  const toX = scroolly._default(rule, 'maxWidth', 'infinity');
  const meter = scroolly._default(scroolly.options, 'meter');
 ...
 * }
 */
scroolly.isRuleActive = (rule, $element, $container) => {
  const checkinWidth = scroolly.isRuleInActiveWidthRange(rule);
  if (!checkinWidth) {
    return false;
  }

  const ruleDirection = scroolly._default(rule, 'direction', 0);
  const scrollDirection = scroolly.direction;
  ...
  const fromY = scroolly._default(rule, 'from', '0');
  const toY = scroolly._default(rule, 'to', 'finish');

  const toTop = scroolly.cmpCoords(fromY, $element, $container);
  if (toTop > 0) {
    return false;
  }

  const toBottom = scroolly.cmpCoords(toY, $element, $container);
  if (toBottom <= 0) {
    return false;
  }
 ...
 */
scroolly.getScrollLayoutLength = () =>
  !Object.keys
    ? $.map(scroolly.scrollLayout, () => 1).length
 ...
 */
scroolly.addItem = (id, $element, rules, $container) => {
  if (!$element.length) {
    return false;
  }
  ...
  cssOnScroll = (element, offset, length, rule) => {
    const progress = offset / length;
    const fromCss = scroolly._default(rule, 'cssFrom');
    const toCss = scroolly._default(rule, 'cssTo');
    const css = {};
    let fromProp;
    let toProp;

    for (const property in fromCss) {
  ...
  const item = scroolly._default(scroolly.scrollLayout, id);
  if (item) {
...
scroolly.factory = ($element, rules, $container, id) => {
  scroolly.init();
 ...
 */
scroolly.processStickyItemRange = ($element, rule = {}) => {
  const $bottomContainer = scroolly._default(
    rule,
    'bottomContainer',
    $('body')
  );
  const mode = scroolly._default(rule, 'mode');
  const offsetTop = scroolly._default(rule, 'offsetTop', 0);
  const offsetBottom = scroolly._default(rule, 'offsetBottom', 0);
  let itemHeight =
  ...
  const offset_1 = Math.round(
    $element.offset().top - parseInt($element.css('margin-top'))
  );
  const offset_2 = Math.round(
 ...
 */
scroolly.getProgress = (offset, length) => {
  const relative = offset / length;
  return {
    offset,
    length,
    relative,
 ...
 * @returns {Number}
 */
scroolly.getTransitionFloatValue = (start, stop, progress) => {
  if (progress <= 0) {
    return start;
  }
 ...
 * @returns {Number}
 */
scroolly.getTransitionIntValue = (start, stop, progress) =>
  Math.round(scroolly.getTransitionFloatValue(start, stop, progress));
 ...
 * @returns {Number}
 */
scroolly.getTransitionColorValue = (start, stop, progress) => {
  if (progress <= 0) {
    return start;
  }
  ...
  const startRGB = scroolly.hashColor2rgb(start);
  const stopRGB = scroolly.hashColor2rgb(stop);
  const r = scroolly.getTransitionIntValue(startRGB[0], stopRGB[0], progress);
  const g = scroolly.getTransitionIntValue(startRGB[1], stopRGB[1], progress);
  const b = scroolly.getTransitionIntValue(startRGB[2], stopRGB[2], progress);
 ...
 * @returns {Number}
 */
scroolly.getTransitionValue = (start, stop, progress) => {
  if (progress <= 0) {
    return start;
  }
  ...
  const re = /(\d*\.\d+)|(\d+)|(#[0-9a-f]{6})|(#[0-9a-f]{3})/gi;
  const stops = `${stop}`.match(re);

  return `${start}`.replace(re, (value, float, int, color6, color3) => {
    //            console.dir({'replace callback args':arguments, stops: stops, called: called});
    const currentStop = stops[called];
 ...
 * @returns {boolean}
 */
scroolly.onScroll = (force) => {
  //        var scrollPos = $(document).scrollTop(); // Y-coord that is checked against fromY & toY
  const scrollPos = scroolly.body.scrollTop(); // Y-coord that is checked against fromY & toY

  if (!force && scrollPos === scroolly.scrollTop) {
    return false;
  }

  const prevPos = scroolly.scrollTop;
  const prevDirection = scroolly.direction;
...
scroolly.cssPrefix = (key) => scroolly.theDashedCSSPrefix + key;

scroolly.extendCssWithPrefix = (cssObj) => {
  const cssExt = {};
  let prop;
  let re;
  let m;
...
scroolly.factorySticky = (
  $element,
  params,
  id = `${$element[0].tagName}_${scroolly.getScrollLayoutLength()}`
) => {
